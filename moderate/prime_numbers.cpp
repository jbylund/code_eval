...
int main(int argc, char* argv[])
{
    ...
    string line;
    vector <unsigned int> questions(0);
    while(input_file.good())
    {
        getline(input_file,line);
        if(0 == line.length())
            continue;
        questions.push_back(atoi(line.c_str()));
    }

    vector <unsigned int> primes(0);
    primes_with_wheel(max(questions),primes);
    for(unsigned int i = 0; i < questions.size(); ++i)
    {
        unsigned int limit = questions[i];
        // print the primes
        unsigned int j = 0;
        while(j+1 < primes.size() && primes[j+1] < limit)
        {
            cout << primes[j] << ",";
            ++j;
        }
        if(primes[j] < limit)
            ...
            cout << "There are no primes less than " << limit << endl;
    }
    return 0;
}
